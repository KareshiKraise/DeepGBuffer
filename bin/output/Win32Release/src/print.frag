...
{	
	...
	if(gl_FragCoord.x < 640.f &&  gl_FragCoord.y > 360.f )
	{
		col = texture(fbcol, vec3(tex, 0));
	}
	else if (gl_FragCoord.x > 640.f &&  gl_FragCoord.y > 360.f )
	{
		col = texture(fbcol, vec3(tex, 1));
	}
	else if (gl_FragCoord.y < 360.f )
	{		
		vec3 val = texture(occlusion, tex).rgb;
		col = vec4(val, 1.0f);
